...
- id: pb1
  ...
  can_device: /dev/serial/by-id/usb-…-if00   # driver kwargs: PowerBoard(can_device=..., bitrate=..., node_id=...)
  bitrate: 1000000
  node_id: 20
```

Use the `/dev/serial/by-id/…` form for the adapter path — it is stable across
reboots and USB ports, unlike `/dev/ttyUSB0`. `guppi rack devices scan` prints
the attached hardware with these stable paths, ready to paste into the config.

A vendor-library device is the same shape — an MCC USB-1608G multifunction DAQ
(`pip install 'guppi-rack[daq]'` for the `uldaq` bindings):

```yaml
- id: daq1
  name: "Bench DAQ"
  type: MccUsb1608G
  role: device                   # rack equipment, not the thing under test
